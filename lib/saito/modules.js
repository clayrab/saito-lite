class Mods {
  ...
  static maybeRun(someMethod) {
    return function () {
      if(this.alwaysRun || this.browser_active) {
        let returnVal = someMethod.apply(this, arguments);
        return returnVal;
      } else {
        console.log("******************************************* blocked " + this.name + " method " + someMethod.name)
      }
    };
  }
  ...
  attachEvents() {
    for (let imp = 0; imp < this.mods.length; imp++) {
      if (this.mods[imp].browser_active == 1) {
        ...
        Mods.maybeRun(this.mods[imp].attachEvents).call(this.mods[imp], this.app);
      }
    }
    return null;
  }
  ...
  async initialize() {
    ...
    //
    for (let i = 0; i < this.mods.length; i++) {
      await Mods.maybeRun(this.mods[i].initialize).call(this.mods[i], this.app);
  ...
  initializeHTML() {
    for (let icb = 0; icb < this.mods.length; icb++) {
      if (this.mods[icb].browser_active == 1) {
        ...
        Mods.maybeRun(this.mods[icb].initializeHTML).call(this.mods[icb], this.app);
      }
    }
    return null;
  }
